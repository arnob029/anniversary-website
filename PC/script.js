...
images.forEach((img, index) => {
  ...
  img.style.left = 300 + x - 60 + "px";
  img.style.top = 300 + y - 60 + "px";
});
...
function createPetal() {
  ...
  petal.style.left = Math.random() * thirdPage.offsetWidth + "px";
  petal.style.fontSize = (15 + Math.random() * 25) + "px";
  petal.style.animationDuration = (4 + Math.random() * 4) + "s";

  thirdPage.appendChild(petal);
...
function typeSentence() {
  ...
  if (charIndex < currentSentence.length) {
    // Continue typing this sentence
    setTimeout(typeSentence, 60); // typing speed
  } else {
    // Full sentence typed → wait 1.2s → next sentence
    setTimeout(() => {
      sentenceIndex = (sentenceIndex + 1) % sentences.length; // loop
      charIndex = 0;
      typeSentence();
    }, 1200); // pause before next sentence
  }
}
...
toThirdPage.addEventListener("click", () => {
  ...
  thirdPage.classList.add("show-page");
});
...
if (toSecondPage) {
  toSecondPage.addEventListener("click", () => {
    ...
    thirdPage.classList.remove("show-page");
    secondPage.style.display = "flex";
    secondPage.classList.add("show-page");
    if (music) {
      music.pause();
